                    ...
                    for pool_type, pool_values in c.SWAPS_HEX.items():
                        if tx_log_topic in pool_values:
                            ...
                            if pool_type == "V2_POOL":
                                if len(swap_data) >= 256:  # Pastikan swap_data cukup panjang
                                    amount0_in, amount1_in, amount0_out, amount1_out = [int(swap_data[i:i+64].hex(), 16) for i in range(0, 256, 64)]
                                else:
                                    # Tangani kasus di mana swap_data tidak valid
                                    print("[!] swap_data tidak valid untuk V2_POOL")
                                ...
                                amount0 = hex_to_decimal(hex_string=swap_data[0:64].hex())  # Pastikan untuk mengonversi bytes ke hex string
                                amount1 = hex_to_decimal(hex_string=swap_data[64:128].hex())  # Pastikan untuk mengonversi bytes ke hex string
